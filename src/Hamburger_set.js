         ...
         <Button className="order" variant="contained" onClick={handleOpen}>주문하기</Button>
            ...
            aria-describedby="modal-modal-description">
                ...
                <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                    <div>A버거 : 7,600원 * {count+(minus_count)}개 = "{5400*(count+(minus_count))}"원</div>
                    <div>B버거 : 6,500원 * {count2+(minus_count2)}개 = "{4900*(count2+(minus_count2))}"원</div>
                    <div>C버거 : 10,900원 * {count3+(minus_count3)}개 = "{8900*(count3+(minus_count3))}"원</div>
                    <div>D버거 : 6,500원 * {count4+(minus_count4)}개 = "{4000*(count4+(minus_count4))}"원</div> <br />
